...
namespace NotesFor.HtmlToOpenXml
{
	...
	public sealed class HtmlDocumentStyle
	{
		...
		internal void PrepareStyles(MainDocumentPart mainPart)
		{
            ...
            RunFonts runNormalTextFonts = new RunFonts() { Ascii = "Arial", HighAnsi = "Arial", ComplexScript = "Arial" };
			...
			foreach (var s in styles.Elements<Style>())
			{
				...
				if (n != null)
				{
                    ...
                    {
                        //ibrahim
                        s.StyleRunProperties.Append(runNormalTextFonts);
                        s.StyleRunProperties.Append(fontNormalTextSize);
                        s.StyleRunProperties.Append(fontComplexNormalTextSize);
                        s.StyleRunProperties.Append(rightToLeftText);
                        //Languages languages8 = new Languages() { Bidi = "ar-EG" };

                        //s.StyleRunProperties.Append(languages8);
